 ...
 * @author adrian
 */
@WebServlet(name = "opcionesListar", urlPatterns = {"/opcionesListar"})
public class opcionesListar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("boton") != null) {
            ...
            switch (request.getParameter("boton").charAt(0)) {
                ...
                case '1':
                    listaHoteles = hotelDAO.getHoteles();
                    if (!listaHoteles.isEmpty()) {
                        request.setAttribute("listaHoteles", listaHoteles);
                        url = "JSP/Admin/visualizar/listarHoteles.jsp";
                    }

                    break;

                case '2':
                    listaActividades = actividadesDAO.getActividades();
                    if (!listaActividades.isEmpty()) {
                        request.setAttribute("listaActividades", listaActividades);
                        url = "JSP/Admin/visualizar/listarActividades.jsp";

                    }
                    break;
                    
                 case '3':
                    listaReservas = reservasDAO.getReservas();
                    if (!listaReservas.isEmpty()) {
                        request.setAttribute("listaReservas", listaReservas);
                        url = "JSP/Admin/visualizar/listarReservas.jsp";
